...
testthat::expect_equal(tau.hagg, tau.hagg1, tolerance=1e-07) # equal up to 1e-07 but results are 
# different at 1e-08.
})
```
...
central = "median")
round(attr(res.cr,"p_interval"), digits = 3) # p-value range [0·000, 0·013]
...
par(xpd = TRUE)
text(latex2exp::TeX('$\\textit{p}$-value$\\in\\lbrack$0,0·013$\\rbrack$'), x = 85, y = 1.4)
dev.off()
...
par(xpd = TRUE)
mtext(side = 3, text = latex2exp::TeX('p-value$\\in\\lbrack$ 0,0·013 $\\rbrack$'), 
outer = 0, cex = 2, lwd = 4)
dev.off()
```
...
for (i in 1:100) {
  lines(end.set, tauCItmp100noinfs[i,], col = scales::alpha("green", alpha = 0.2), lwd = 4)
}
...
lty=c(1,1,1,1,1,2), lwd = c(6,6,30,6,30,6), pch = c(124,NULL,NULL,NULL,NULL,NULL), cex=1.05, 
yjust = 0.5)
dev.off()
```
