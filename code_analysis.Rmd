...
The actual tau functions as coded in the C language can be found in `./tauodds/src/*.cpp` based on adaptations from an older fork (v0.3.7) of the `IDSpatialStats` package [@LesslerGiles].
...
test_that("comparing Lessler RISB, RISB, MPSB and MMPSB, against non-computer calcs",{
set.seed(seed = 1)
...
expect_equal(object = lessler.RISB[,3], expected = c(3.5,0.0)) #PASS
...
expect_equal(object = pollington.RISB, expected = c(3.5,0.0), tolerance = 1e-06) #PASS

pollington.MPSB = tauodds::getTau2Loh(toy[,"x"], toy[,"y"], toy[,"t"], toy.r.min, 
  toy.r.max, as.integer(inds), 0, 4) 
expect_equal(object = pollington.MPSB, expected = c(0.5,0.25))

pollington.MMPSB = tauodds::getTau2Lohv2(toy[,"x"], toy[,"y"], toy[,"t"], toy.r.min, 
  toy.r.max, as.integer(inds), 0, 4)
expect_equal(object = pollington.MMPSB, expected = c(13/7,13/35), 
             tolerance = 1e-03)
})
```
