...
expect_equal(tau.hagg, tau.hagg1, tolerance=1e-07) # equal up to 1e-07 but results are 
# different at 1e-08.
```
...
load(file = "tau.hagg.RData")
curveset = GET::create_curve_set(list(r = r.max, obs = tau.hagg, 
                                      sim_m = as.matrix(hag.permute)[,c(-1,-2)]))
...
plot(NULL, xlim = c(0,120), ylim = c(min(res.cr$lo, res.cr$obs),max(res.cr$obs)), 
...
r.max1 = r.max[1:46]
res.cr1 = res.cr[1:46,]
hag.permute1 = hag.permute[1:46,c(-1,-2)]

pdf("get.ga.pdf", width = 7*4, height = 7*4, pointsize = 12*4)
plot(NULL, xlim = c(10,100), ylim = c(min(res.cr1$lo,res.cr1$obs),max(res.cr1$obs)), xaxt = "n", 
